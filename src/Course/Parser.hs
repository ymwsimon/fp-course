...
import Data.Char
import Course.Traversable (Traversable(traverse), sequenceA)
...
(|||) ::
  ...
  -> Parser a
  -> Parser a
pf ||| ps = P $ \input -> case parse pf input of
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  f =<< pa = P $ \input -> case parse pa input of
                              Result i a -> let P np = f a in
                                                np i
...
phoneParser ::
  Parser Chars
-- phoneParser = valueParser Nil
phoneParser = lift2 (++) (digit .:. phoneBodyParser) $ is '#' *> valueParser Nil
